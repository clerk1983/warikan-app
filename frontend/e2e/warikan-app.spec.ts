...
test.describe("割り勘アプリ", () => {
  ...
  test.describe('グループ作成', () => {
    test("グループが作成され支出登録ページに遷移する", async ({ page }) => {
      const groupNameInput: Locator = page.getByLabel('グループ名')
      await groupNameInput.fill("group1")
      const memberListInput = page.getByLabel('メンバー')
      await memberListInput.fill('Alice,Bob')
      const submitButton = page.getByRole('button')
      await submitButton.click()

      await expect(page).toHaveURL(/.*group\/group1/)
    })
    test("バリデーションエラーが存在する場合グループが作成されずページ遷移しない", async ({ page }) => {
      const submitButton = page.getByRole('button')
      await submitButton.click()
  ...
  test.describe("支出登録機能", () => {
    ...
    test("支出が登録され精算リストが更新される", async ({ page }) => {
      ...
      await memberSelect.selectOption('Bob')
      const submitButton = page.getByRole("button")
      await submitButton.click()

      await expect(page).toHaveURL(/.*group\/group1/)
      await expect(page.getByRole('list')).toHaveText("Alice → Bob500円")

    })
    test("バリデーションエラーがある場合精算リストが更新されない", async ({ page }) => {
      const submitButton = page.getByRole('button')
      await submitButton.click()
